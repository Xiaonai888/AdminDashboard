...
function BuyerInfo({ buyer }) {
  return (
    <div>
      ...
      <div className="small">Name: <span className="strong">{buyer?.name || '-'}</span></div>
      <div className="small">Phone: <span className="strong">{buyer?.phone_number || '-'}</span></div>
...
function BooksInfo({ items }) {
  ...
  return (
    <div>
      <div className="strong">Books</div>
      <div className="book-list" style={{ marginTop: 8 }}>
        {safeItems.map((item, index) => (
          <div className="book-item" key={`${item.product_id || index}`}>
            <div className="book-cover">
              {item.cover_url ? <img src={item.cover_url} alt={item.title || 'Book'} /> : null}
            </div>
            <div>
              <div className="small"><span className="strong">{item.title || 'Book'}</span></div>
              <div className="small">Qty: {item.quantity || 1} · {formatUsd(item.unit_price_usd)} each</div>
            </div>
          </div>
        ))}
      </div>
...
export default function ShadowMallOrdersPage() {
  ...
  async function fetchOrders(nextPage = page) {
    try {
      ...
      const response = await fetch(`${API_URL}/api/shadow-mall/admin/orders?${params.toString()}`, {
        headers: {
      ...
      if (!response.ok || data.ok === false) {
        throw new Error(data.message || 'Failed to load Shadow Mall orders')
      }
      ...
      setMessage(error.message || 'Failed to load Shadow Mall orders')
      setOrders([])
    } finally {
      setLoading(false)
    }
  ...
  async function updateOrderStatus(orderId, nextStatus) {
    try {
      ...
      const token = getAdminToken()
      const response = await fetch(`${API_URL}/api/shadow-mall/admin/orders/${orderId}/status`, {
        method: 'PATCH',
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
